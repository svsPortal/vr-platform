const V=[
{name:"星乃ゆら",emoji:"🌸",subs:"428人",tag:"雑談・ゲーム",live:true},
{name:"月城ルナ",emoji:"🌙",subs:"812人",tag:"歌・ASMR",live:true},
{name:"猫宮こはく",emoji:"🐱",subs:"196人",tag:"ゲーム",live:false},
{name:"空色ミオ",emoji:"☁️",subs:"1,240人",tag:"歌・雑談",live:false}
];let page="home",balance=2840;const app=document.getElementById("app");
function topbar(back=false,title=""){return '<div class="top">'+(back?'<button class="back" onclick="go(\'home\')">‹</button>':'<div class="logo">VR</div>')+(title?'<b class="topTitle">'+title+'</b>':'')+'<div class="spacer"></div><button class="icon">⌕</button><button class="icon">♧</button><button class="icon">☺</button></div>'}
function nav(){return '<nav class="bottom">'+[["home","⌂","ホーム"],["live","▣","配信"],["shorts","▶","Shorts"],["discover","⌕","発掘"],["vr","♧","VR"],["mypage","♙","マイページ"]].map(x=>'<button class="nav '+(page===x[0]?'active':'')+'" onclick="go(\''+x[0]+'\')"><span>'+x[1]+'</span>'+x[2]+'</button>').join("")+'</nav>'}
function card(v,i){return '<article class="card" onclick="profile('+i+')"><div class="thumb">'+v.emoji+(v.live?'<b class="live">● LIVE</b>':'')+'</div><div class="card-body"><div class="name">'+v.name+'</div><div class="meta">登録者 '+v.subs+' ・ '+v.tag+'</div></div></article>'}
